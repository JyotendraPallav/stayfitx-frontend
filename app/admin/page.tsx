...
function formatDays(dow: number[]) {
  return dow.sort().map(d => DAY_NAMES[d]).join(' · ');
}
...
export default function AdminPage() {
  ...
  // ── Summary KPI memos ──────────────────────────────────────────────────────
  const sumAllWeek = useMemo(() => Object.values(summaryWeekSessions).flat(), [summaryWeekSessions]);
  const sumScheduled = useMemo(() => sumAllWeek.filter(s => s.status === 'scheduled'), [sumAllWeek]);
  const sumCancelled = useMemo(() => sumAllWeek.filter(s => s.status === 'cancelled'), [sumAllWeek]);
  const sumRescheduled = useMemo(() => sumAllWeek.filter(s => s.status === 'rescheduled'), [sumAllWeek]);
  const sumCancelRate = useMemo(() =>
    sumAllWeek.length > 0 ? Math.round((sumCancelled.length / sumAllWeek.length) * 100) : 0,
    [sumAllWeek, sumCancelled]);
  const sumActiveClients = useMemo(() => [...new Set(summarySeries.map(s => s.client_name))].length, [summarySeries]);
  const sumTrainerCounts = useMemo(() =>
    trainers.map(t => ({
      trainer: t,
      count: sumScheduled.filter(s => s.trainer_name === t.name).length,
      cancelled: sumCancelled.filter(s => s.trainer_name === t.name).length,
    })).sort((a, b) => b.count - a.count),
    [trainers, sumScheduled, sumCancelled]);
  const sumMaxCount = useMemo(() => Math.max(1, sumTrainerCounts[0]?.count || 1), [sumTrainerCounts]);
  const sumDayTotals = useMemo(() =>
    weekDays.map((d, i) => ({
      label: DAY_NAMES_SHORT[i],
      date: d,
      count: (summaryWeekSessions[d] || []).filter(s => s.status === 'scheduled').length,
    })),
    [weekDays, summaryWeekSessions]);
  const sumMaxDay = useMemo(() => Math.max(1, ...sumDayTotals.map(d => d.count)), [sumDayTotals]);
  const sumHourCounts = useMemo(() => {
    const hc: Record<number, number> = {};
    sumScheduled.forEach(s => {
      const h = new Date(s.start_datetime).getHours();
      hc[h] = (hc[h] || 0) + 1;
    });
    return hc;
  }, [sumScheduled]);
  const sumPeakHourEntry = useMemo(() =>
    Object.entries(sumHourCounts).sort((a, b) => b[1] - a[1])[0],
    [sumHourCounts]);
  const sumPeakHour = useMemo(() =>
    sumPeakHourEntry ? parseInt(sumPeakHourEntry[0]) : null,
    [sumPeakHourEntry]);
  const sumPeakLabel = useMemo(() =>
    sumPeakHour === null ? '—' : sumPeakHour < 12 ? `${sumPeakHour}:00 AM` : sumPeakHour === 12 ? '12:00 PM' : `${sumPeakHour - 12}:00 PM`,
    [sumPeakHour]);
  const sumIdleTrainers = useMemo(() => sumTrainerCounts.filter(t => t.count === 0), [sumTrainerCounts]);
  const sumHighCancellers = useMemo(() =>
    sumTrainerCounts.filter(t => {
      const total = t.count + t.cancelled;
      return total > 0 && (t.cancelled / total) > 0.2;
    }),
    [sumTrainerCounts]);

  // ── Trainer detail memos ───────────────────────────────────────────────────
  const tdIdx = useMemo(() => selectedTrainer ? trainers.findIndex(t => t.id === selectedTrainer.id) : -1, [selectedTrainer, trainers]);
  const tdColor = useMemo(() => TRAINER_PALETTE[tdIdx >= 0 ? tdIdx % TRAINER_PALETTE.length : 0], [tdIdx]);
  const tdScheduled = useMemo(() => trainerWeekSessions.filter(s => s.status === 'scheduled'), [trainerWeekSessions]);
  const tdCancelled = useMemo(() => trainerWeekSessions.filter(s => s.status === 'cancelled'), [trainerWeekSessions]);
  const tdRescheduled = useMemo(() => trainerWeekSessions.filter(s => s.status === 'rescheduled'), [trainerWeekSessions]);
  const tdCancelRate = useMemo(() =>
    trainerWeekSessions.length > 0 ? Math.round((tdCancelled.length / trainerWeekSessions.length) * 100) : 0,
    [tdCancelled, trainerWeekSessions]);
  const tdWeekClients = useMemo(() => [...new Set(trainerWeekSessions.map(s => s.client_name))], [trainerWeekSessions]);
  const tdByDay = useMemo(() => {
            ...
            {summaryLoading ? (
              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    { label: 'Active Clients', value: sumActiveClients, icon: Users, color: '#A78BFA', sub: 'across all trainers' },
                    { label: 'Sessions This Week', value: sumScheduled.length, icon: CalendarDays, color: '#34D399', sub: `+${sumRescheduled.length} rescheduled` },
                    { label: 'Total Trainers', value: trainers.length, icon: UserCheck, color: '#38BDF8', sub: `${sumIdleTrainers.length > 0 ? sumIdleTrainers.length + ' idle' : 'all active'}` },
                    { label: 'Cancel Rate', value: `${sumCancelRate}%`, icon: AlertTriangle, color: sumCancelRate > 20 ? '#F87171' : sumCancelRate > 10 ? '#FCD34D' : '#34D399', sub: `${sumCancelled.length} cancelled` },
                ...
                <div className="glass rounded-2xl p-5">
                  ...
                  <div className="flex items-end gap-2 h-24">
                    {sumDayTotals.map(({ label, date, count }) => {
                      const isToday = date === format(new Date(), 'yyyy-MM-dd');
                      const heightPct = sumMaxDay > 0 ? Math.max(8, Math.round((count / sumMaxDay) * 100)) : 8;
                      return (
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="glass rounded-2xl p-4 flex flex-col gap-1">
                    ...
                    <p className="text-xl font-extrabold text-white">{sumPeakLabel}</p>
                    <p className="text-[10px]" style={{ color: 'var(--muted)' }}>
                      {sumPeakHourEntry ? `${sumPeakHourEntry[1]} sessions` : 'No data yet'}
                    </p>
                  </div>
                  <div className="glass rounded-2xl p-4 flex flex-col gap-1">
                    <div className="flex items-center gap-2 mb-1">
                ...
                {/* Alerts */}
                {(sumIdleTrainers.length > 0 || sumHighCancellers.length > 0) && (
                  <div className="glass rounded-2xl p-5 space-y-3" style={{ border: '1px solid rgba(245,158,11,0.2)', background: 'rgba(245,158,11,0.04)' }}>
                    ...
                    {sumHighCancellers.map(({ trainer, cancelled, count }) => (
                      <div key={trainer.id} className="flex items-center gap-3">
                        <div className="w-6 h-6 rounded-full flex items-center justify-center text-[9px] font-bold text-white flex-shrink-0"
                          style={{ background: TRAINER_PALETTE[trainers.findIndex(t => t.id === trainer.id) % TRAINER_PALETTE.length].bg }}>
                          {initials(trainer.name)}
      ...
      {selectedTrainer && (
        <div className="fixed inset-0 z-50 flex items-end" style={{ background: 'rgba(0,0,0,0.7)' }}
          ...
          <div className="glass w-full rounded-t-3xl pb-10 max-h-[90vh] overflow-y-auto"
            ...
            {/* Colored header band */}
            <div className="p-6 pb-5" style={{ background: `linear-gradient(135deg, ${tdColor.solid}22, transparent)`, borderBottom: `1px solid ${tdColor.solid}30` }}>
              ...
              <div className="flex items-center gap-4">
                <div className="w-16 h-16 rounded-2xl flex items-center justify-center text-xl font-bold text-white flex-shrink-0"
                  style={{ background: tdColor.bg, boxShadow: `0 8px 24px ${tdColor.shadow}` }}>
                ...
                <>
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {[
                      ...
                      { label: 'This week', value: tdScheduled.length, color: '#34D399' },
                      { label: 'Clients', value: tdWeekClients.length, color: '#38BDF8' },
                      { label: 'Cancel %', value: `${tdCancelRate}%`, color: tdCancelRate > 20 ? '#F87171' : '#FCD34D' },
                    ].map(stat => (
                  ...
                  {trainerWeekSessions.length > 0 && (
                    <div className="rounded-xl p-4 space-y-2" style={{ background: 'rgba(0,0,0,0.2)', border: '1px solid var(--border)' }}>
                      ...
                      {[
                        { label: 'Scheduled', count: tdScheduled.length, cls: 'badge-green' },
                        { label: 'Rescheduled', count: tdRescheduled.length, cls: 'badge-amber' },
                        { label: 'Cancelled', count: tdCancelled.length, cls: 'badge-red' },
                      ].map(row => (
                  ...
                  {trainerSeries.length === 0 && trainerWeekSessions.length === 0 && !trainerDetailLoading && (
